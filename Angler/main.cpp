...
TEST_CASE("no skillful angler more contests ", "t6.txt") {
    string id;
...
TEST_CASE("1 angler more contests", "t3.txt") {
    AnglerEnor t("t3.txt");
    t.first();
    CHECK_FALSE(t.current().skillful);
}

TEST_CASE("1 angler did not catch 2 catfishes on his first contest", "t3.txt") {
    AnglerEnor t("t3.txt");
    t.first();
    CHECK_FALSE(t.current().skillful);
}

TEST_CASE("1 angler did not catch 2 catfishes on his last contest", "t9.txt") {
    AnglerEnor t("t9.txt");
    t.first();
    CHECK_FALSE(t.current().skillful);
}

TEST_CASE("first angler did not catch 2 catfishes", "t11.txt") {
    AnglerEnor t("t11.txt");
    t.first();
    CHECK_FALSE(t.current().skillful);
}

TEST_CASE("first angler caught catfish on only one competition", "t10.txt") {
    AnglerEnor t("t10.txt");
    t.first();
    CHECK_FALSE(t.current().skillful);
}

TEST_CASE("the first angler is skillful", "t7.txt") {
    AnglerEnor t("t7.txt");
    t.first();
    CHECK(t.current().skillful);
}
